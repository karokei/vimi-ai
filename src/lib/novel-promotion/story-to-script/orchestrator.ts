...
function parseClipArray(responseText: string): Record<string, unknown>[] {
    ...
    if (firstBracket !== -1 && lastBracket !== -1 && lastBracket > firstBracket) {
        ...
        try {
            const parsed = JSON.parse(escapeControlCharsInJsonStrings(arrayStr))
...
export async function runStoryToScriptOrchestrator(
    ...
    onLog?.('Starting Step 1: Character/Location Analysis (Parallel)')
    const [
    ...
    onLog?.('Starting Step 2: Content Segmentation')
    ...
    const splitPromptBase = applyTemplate(promptTemplates.clipPromptTemplate, {
        ...
        characters_introduction: charactersIntroduction || 'No character introductions',
    })
    ...
    for (let attempt = 1; attempt <= MAX_SPLIT_BOUNDARY_ATTEMPTS; attempt += 1) {
        ...
        try {
            ...
            lastBoundaryError = new Error(`Boundary matching failed at ${failedAt.clipId}`)
        } catch (e) {
            lastBoundaryError = e as Error;
        }
    }
    ...
    onLog?.('Starting Step 3: Screenplay Conversion', { clipCount: clipList.length })
...
export async function executeStoryToScript(
    content: string,
    baseCharacters: string[],
    baseLocations: string[],
    baseCharacterIntroductions: Array<{ name: string; introduction?: string | null }>,
    ...
    return runStoryToScriptOrchestrator({
        content,
        baseCharacters,
        baseLocations,
        baseCharacterIntroductions,
        promptTemplates: {
            characterPromptTemplate: PromptManager.getTemplate(PROMPT_IDS.NP_AGENT_CHARACTER_PROFILE),
            locationPromptTemplate: PromptManager.getTemplate(PROMPT_IDS.NP_AGENT_CLIP), // Placeholder or specific
            clipPromptTemplate: PromptManager.getTemplate(PROMPT_IDS.NP_AGENT_CLIP),
            screenplayPromptTemplate: PromptManager.getTemplate(PROMPT_IDS.NP_SCREENPLAY_CONVERSION),
        },
